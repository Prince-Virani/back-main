...
<div id="sidebar" class="app-sidebar">
	...
	<div class="app-sidebar-content" data-scrollbar="true" data-height="100%">
		...
		<div class="menu">
            ...
            use Illuminate\Support\Facades\Gate; // To check permissions
            ...
            function renderSubMenu($value, $currentUrl) {
                ...
                foreach ($value as $key => $menu) {
                    ...
                    $menuUrl = (!empty($menu['url']) && Route::has($menu['url'])) ? route($menu['url']) : '#';
                    ...
                    if (!empty(config('settingSidebar.activeUrl'))) {
                        $active = (config('settingSidebar.activeUrl') == $menuUrl) ? 'active' : '';
                    }
                    if ($active) {
                    ...
                    $hasPermission = !empty($menu['permission']) ? auth()->user()->can($menu['permission']) : true;
            ...
            foreach (config('sidebar.menu') as $key => $menu) {
                ...
                $active = (!empty($menu['url']) && $currentUrl == $menuUrl) ? 'active' : '';
                $active = (empty($active) && !empty($GLOBALS['parent_active'])) ? 'active' : $active;

                if (!empty(config('settingSidebar.activeUrl'))) {
                    $active = (!empty($menu['url']) && config('settingSidebar.activeUrl') == $menuUrl) ? 'active' : '';
                    $active = (empty($active) && !empty($GLOBALS['parent_active'])) ? 'active' : $active;
                }
