    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-2">
                <li class="breadcrumb-item"><a href="#" class="text-decoration-none text-primary fw-medium"><i class="fas fa-shield-alt me-1"></i>Roles</a></li>
                <li class="breadcrumb-item active text-muted">Create Role</li>
            </ol>
        </nav>
        <h1 class="page-header mb-0 fw-bold text-dark">Create New Role</h1>
        <p class="text-muted mb-0">Define a role and assign permissions</p>
    </div>
    <div class="ms-auto">
        <a href="{{ route('roles.index') }}" class="btn btn-outline-secondary"><i class="fas fa-arrow-left me-2"></i>Back</a>
    </div>
</div>
    ...
    <form action="{{ route('roles.store') }}" method="POST" id="roleForm">
        ...
        @php
            ...
            foreach ($permission as $perm) {
                ...
                $grouped[$key][] = ['id' => $perm->id, 'name' => $name];
            }
            ksort($grouped);
            $oldSelected = collect(old('permission', []))->map(fn($v)=> (string)$v)->toArray();
        @endphp
            ...
            <div class="col-12 col-lg-4">
                <div class="info-card h-100">
                    <div class="card-header">
                        <h5 class="mb-0 fw-semibold text-dark"><i class="fas fa-user-tag me-2 text-primary"></i>Role Information</h5>
                    </div>
                    <div class="card-body">
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="name" value="{{ old('name') }}" class="form-control border-start-0 ps-0" placeholder="e.g., Admin, Editor, Viewer" id="roleName">
                            </div>
                        ...
                        <div class="action-section mt-4">
                            <button type="submit" class="btn btn-primary btn-lg w-100 fw-semibold"><i class="fas fa-save me-2"></i>Create Role</button>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="permissions-card h-100">
                    <div class="card-header">
                        <div class="d-flex align-items-center justify-content-between">
                            <h5 class="mb-0 fw-semibold text-dark"><i class="fas fa-key me-2 text-warning"></i>Permissions Management</h5>
                            <div class="header-actions">
                                <button type="button" class="btn btn-sm btn-outline-primary" id="expandAll"><i class="fas fa-expand-alt me-1"></i>Expand</button>
                                <button type="button" class="btn btn-sm btn-outline-secondary" id="collapseAll"><i class="fas fa-compress-alt me-1"></i>Collapse</button>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="row g-3 align-items-end">
                                ...
                                <div class="col-md-6">
                                    <div class="bulk-actions">
                                        <button type="button" class="btn btn-success btn-sm" id="selectFiltered"><i class="fas fa-check-circle me-1"></i>Select Filtered</button>
                                        <button type="button" class="btn btn-outline-danger btn-sm" id="clearFiltered"><i class="fas fa-times-circle me-1"></i>Clear Filtered</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div id="c-{{ $gid }}" class="accordion-collapse collapse">
                                        <div class="group-body">
                                            <div class="group-actions mb-3">
                                                <button type="button" class="btn btn-sm btn-outline-success group-select" data-target="{{ $gid }}"><i class="fas fa-check-square me-1"></i>Select All</button>
                                                <button type="button" class="btn btn-sm btn-outline-secondary group-clear" data-target="{{ $gid }}"><i class="fas fa-square me-1"></i>Clear All</button>
                                            </div>
                                            <div class="permission-grid" data-group="{{ $gid }}">
                                                @foreach($items as $it)
                                                    @php
                                                        $checked = in_array((string)$it['id'], $oldSelected) ? 'checked' : '';
                                                        $safeId = 'perm_'.md5($it['id'].'|'.$it['name']);
                                                    @endphp
                                                    <div class="permission-item" data-name="{{ strtolower($it['name']) }}">
                                                        <label for="{{ $safeId }}" class="permission-label">
                                                            ...
                                                            <div class="permission-content">
                                                                <div class="permission-name">{{ $it['name'] }}</div>
                                                                <div class="permission-checkmark"><i class="fas fa-check"></i></div>
                                                            </div>
                                                        </label>
                                                    </div>
                                                @endforeach
                            ...
                            <div class="row align-items-center">
                                <div class="col-md-6">
                                    <div class="quick-actions">
                                        <button type="button" class="btn btn-outline-primary btn-sm" id="expandAllBottom"><i class="fas fa-expand-alt me-1"></i>Expand All</button>
                                        <button type="button" class="btn btn-outline-secondary btn-sm" id="collapseAllBottom"><i class="fas fa-compress-alt me-1"></i>Collapse All</button>
                                    </div>
                                </div>
...
(function(){
    ...
    function recalcSelected(){
        ...
        chipsToShow.forEach(cb=>{
            ...
            chip.innerHTML=`<span>${label}</span><button type="button" class="x" aria-label="Remove">&times;</button>`;
